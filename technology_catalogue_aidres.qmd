...
Get data on the CO2 consumption of co electrolysis processes (based on Figure 12 and 13 of the final aidres report) + data on the processes to convert hydrogen to Fischer-Tropsch liquid (FT) and hydrogen to methanol (MeOH) based on the processes ‘Hydrogen to Jet’ and ‘Methanol from hydrogen and carbon dioxide’, respectively, of the Danish technology catalogue.

For the AIDRES processes (COEL)FT-MEA and (COEL)MeOH-MEA, the CO2 consumption from the final aidres report is expressed in ton CO2/MWh LLF, which is converted to ton CO2/MWh LLF based on a lower heating value of 11.91 MWh/t. For ((COEL)MeOH)O-MEA, ton CO2/ton LLF is converted to ton CO2/ton olefins by dividing by 2.1552 ton MeOH/ton LLF and by dividing by 0.4377 ton olefins/ton MeOH. Check the 'extra_data_processed.xlsx' file for more info on the calculation.

For the ‘Hydrogen to Jet’ process in the Danish technology catalogue, the capex is expressed in Meur_(MW_FT)_year. To convert this to (eur-yr)/mwh, we multiply by 1000000 eur/Meur and divide by 8760 h/y. The fom is expressed in 'eur_(MWh_FT)' which is already the correct unit. Check the 'extra_data_processed.xlsx' file for more info on the calculation.
--> Remark: we are still going to check these values with our TIMES model!

For the ‘Methanol from hydrogen and carbon dioxide’ in the Danish technology catalogue, the capex is expressed in Meur_(MW_methanol). To convert this to (eur-yr)/mwh, we multiply by 1000000 eur/Meur and divide by 8760 h/y. The fom is expressed in 'keur_(MW_methanol_year)', which is multiplied by 1000 eur/keur and divided by 8760 h/year to convert it to eur/mwh. Check the 'extra_data_processed.xlsx' file for more info on the calculation.
--> Remark: we are still going to check these values with our TIMES model!
...
```{r}
print(extra_data_processes)
      extra_data_processes |> 
        filter(parameter == 'sec') |>
        select('sector_id', 'Industry', 'from_node', 'to_node', 'unit', '2025', '2030', '2040', '2050') -> extra_data_filtered
...
## Costs
The costs of aidres and the danish technology catalogue are expressed in EUR2018 and EUR2020, respectively. These costs are converted to EUR2025 based on the historical inflation rates.
```{r}
extra_data_processes |> 
...
# Capex and cst opex is multiplied with historical and expected (= 2%) inflation rate
...
print(extra_data_life_filtered)
...
write_xlsx(list('ind_production_2018_nuts3' = prod_vol_cap_2018year_nuts3_no_HC_NH3,
                ...
                'ind_production_2018_nuts1' = prod_vol_cap_2018year_no_HC_NH3,
                'ind_production_30_50_nuts1' = prod_vol_cap_allyear_no_HC_NH3,
           ...
           here(output_dir, 'technology_catalogue_ind_nuts1_nuts3_no_HC_NH3.xlsx'))
```
...
write_xlsx(list('ind_production_2018_nuts3' = prod_vol_cap_2018year_nuts3_HC_NH3,
                ...
                'ind_production_2018_nuts1' = prod_vol_cap_2018year_HC_NH3,
                'ind_production_30_50_nuts1' = prod_vol_cap_allyear_HC_NH3,
           ...
           here(output_dir, 'technology_catalogue_ind_nuts1_nuts3_HC_NH3.xlsx'))
```