...
# AIDRES and AIDRES 2.0 Glass and Cement production
...
aidres_2_0_cleaned_cg <- aidres_2_0_energy_all |> 
  select(country_code:country, glass, cement) |> 
...
aidres_2_0_production_country_2018 |> 
  ...
  mutate(mt = round(production_kt_y/1000, 3), .keep = 'unused') |> 
  rename('product' = 'product_id') |> 
  mutate(source = 'aidres')  -> aidres_2_0_production_country_plot_cement
```
...
aidres_2_0_production_country_2018 |> 
  ...
  mutate(product = 'glass') |>
  mutate(mt = round(production_kt_y/1000, 3), .keep = 'unused') |> 
  mutate(source = 'aidres')  -> aidres_2_0_production_country_plot_glass
...
bind_rows(cg_production_allyear_plot |> filter(year == 2018, product == 'cement') |> select(-year), 
          aidres_2_0_production_country_plot_cement) |> 
  ggplot(aes(mt, fct_reorder(country_code, mt, .fun = sum)))+
  #        geom_col(width = 0.5) + 
  geom_line()+
  geom_point(aes(colour = source), size = 1.5)+
#  facet_wrap(~product, ncol = 3)+
  theme_bw()+
  labs(title = 'Cement sector production volume comparison aidres 2.0',
       y = 'Country') +
  theme(legend.position = "bottom")-> p100
p100
ggsave(here(output_dir, 'cement_production_compare_2_0.pdf'), width = 10, height = 8, units = 'in', dpi = 900)
```
...
```{r, fig.width=10,fig.height=7}
bind_rows(cg_production_allyear_plot |> filter(year == 2018, product == 'glass') |> select(-year), 
          aidres_2_0_production_country_plot_glass) |> 
  ggplot(aes(mt, fct_reorder(country_code, mt, .fun = sum)))+
  geom_line()+
  geom_point(aes(colour = source), size = 1.5)+
  theme_bw()+
  labs(title = 'Glass sector production volume comparison aidres 2.0',
       y = 'Country')+
  theme(legend.position = "bottom") -> p110
p110
ggsave(here(output_dir, 'glass_production_compare_2_0.pdf'), width = 10, height = 8, units = 'in', dpi = 900)
```
...
bind_rows(cg_production_allyear_plot |> filter(year == 2018, product == 'cement') |> select(-year), 
          aidres_2_0_production_country_plot_cement) |> 
  pivot_wider(names_from = source, values_from = mt) |> 
  write_xlsx(here(output_dir, 'diff_production_volume_cement_2_0.xlsx'))

bind_rows(cg_production_allyear_plot |> filter(year == 2018, product == 'glass') |> select(-year), 
          aidres_2_0_production_country_plot_glass) |> 
pivot_wider(names_from = source, values_from = mt) |> 
  write_xlsx(here(output_dir, 'diff_production_volume_glass_2_0.xlsx'))
```
...
aidres_2_0_cleaned_cg |> filter(sector == 'cem') |> 
  select(-country, -sector) |> 
  rbind(cem_energy_fec_) |> 
  arrange(country_code, fuel) -> cem_fec_compare_2_0
...
cem_fec_compare_2_0 |> 
  ...
  geom_point(aes(colour = source), size = 2) +
  theme_bw() +
  labs(title = 'Total energy consumption in the cement sector in AidRes 2.0 and JRC-IDEES',
       subtitle = 'IDEES contains lime also',
       y = 'country') +
  theme(legend.position = "bottom") -> p_cem_compare_tot_2_0
 p_cem_compare_tot_2_0
ggsave(here(output_dir, 'cem_fec_tot_compare_2_0.pdf'), width = 10, height = 8, units = 'in', dpi = 900)
```
...
#  filter(fuel != 'total') |> 
  ...
  facet_wrap(~fuel, ncol = 4, scales = "free") +
  theme_bw() +
  labs(title = 'Fuel wise energy consumption in the cement sector in AidRes 2.0 and JRC-IDEES',
       subtitle = 'IDEES contains lime also',
       y = 'country') +
  theme(legend.position = "bottom") -> p_cem_compare_all_2_0

p_cem_compare_all_2_0
ggsave(here(output_dir, 'cem_fec_all_compare_2_0.pdf'), width = 18, height = 8, units = 'in', dpi = 900)
...
cem_fec_compare_2_0 |> 
  ...
  mutate(pj = replace_na(pj, 0)) |> 
  pivot_wider(names_from = source, values_from = pj) |> 
  write_xlsx(here(output_dir, 'diff_fec_cement_2_0.xlsx'))
```
...
aidres_2_0_cleaned_cg |> filter(sector == 'glass') |> 
  select(-country, -sector) |> 
  rbind(glass_energy_fec_) |> 
  arrange(country_code, fuel) -> glass_fec_compare_2_0
```
...
glass_fec_compare |> 
  ...
  geom_point(aes(colour = source), size = 1.5) +
  theme_bw() +
  labs(title = 'Total energy consumption in the glass sector in AidRes and JRC-IDEES',
       y = 'country') +
...
glass_fec_compare_2_0 |> 
  filter(fuel == 'total') |> 
  ggplot(aes(pj, fct_reorder(country_code, pj, .fun = sum))) +
  geom_line()+
  geom_point(aes(colour = source), size = 1.5) +
  theme_bw() +
  labs(title = 'Total energy consumption in the glass sector in AidRes 2.0 and JRC-IDEES',
       y = 'country') +
  theme(legend.position = "bottom") -> p_glass_compare_tot_2_0
p_glass_compare_tot_2_0
ggsave(here(output_dir, 'glass_fec_tot_compare_2_0.pdf'), width = 10, height = 8, units = 'in', dpi = 900)
```
...
glass_fec_compare_2_0 |> 
  ...
  facet_wrap(~fuel, ncol = 4, scales = "free") +
  theme_bw() +
  labs(title = 'Total energy consumption in the glass sector in AidRes and 2.0 JRC-IDEES',
       y = 'country') +
  theme(legend.position = "bottom") -> p_glass_compare_all_2_0
p_glass_compare_all_2_0
ggsave(here(output_dir, 'glass_fec_all_compare_2_0.pdf'), width = 18, height = 8, units = 'in', dpi = 900)
...
glass_fec_compare_2_0 |> 
  ...
  mutate(pj = replace_na(pj, 0)) |> 
  pivot_wider(names_from = source, values_from = pj) |> 
  write_xlsx(here(output_dir, 'diff_fec_glass_2_0.xlsx'))
```
